...
| Package | What |
|---|---|
| `core/` | `@aura/core` — memory store: Neon Postgres + pgvector, local embeddings, `addMemory` / `searchMemory` / … |
...
```
Secrets live in `.env` (root: DATABASE_URL, AURA_USER_ID) and `web/.env.local` (BETTER_AUTH_*). Both are git-ignored.
...
Progress: Steps 0–5 done; Step 6 needs `vercel login` to deploy. See `START_HERE.md` / `BUILD_PLAN.md`.
